			...
			<nav class="limiter-menu-desktop container">
				...
				<div class="wrap-icon-header flex-w flex-r-m">
					...
					<div class="wrap-icon-header flex-w flex-r-m">
						...
						<div class="icon-header-item cl2 hov-cl1 trans-04 p-l-22 p-r-11" id="search-icon">
							<i class="zmdi zmdi-search"></i>
						</div>
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		var allProducts = document.querySelectorAll('#search_filed .isotope-item');

		searchInput.addEventListener('keyup', function() {
			var searchValue = this.value.toLowerCase();

			allProducts.forEach(product => {
				var productTitle = product.querySelector('.js-name-b2').textContent.toLowerCase();
				if (productTitle.includes(searchValue)) {
					product.classList.remove('isotope-hidden');
				} else {
					product.classList.add('isotope-hidden');
					// allProduvts.style.display = 'none';
				}
			});
		});
	});
...
<script>
        $(document).ready(function () {
            ...
            // Add a search event listener to the search input
